...
function Live2d() {
  ...
  class Model {
    ...
    constructor(config) {
      ...
      this.#config = config;
      // 启动异步加载本地模型配置
      this.#loadPromise = this.#loadModelsConfig();
    }
  
    async #loadModelsConfig() {
      try {
        console.log('Loading models config from:', `${this.#apiPath}models.json`);
        const response = await fetch(`${this.#apiPath}models.json`);
        if (!response.ok) {
    ...
    async loadModel(modelId, modelTexturesId, text = "模型加载中...") {
      try {
        ...
        const model = this.#modelsData.models[modelId];
        if (!model) {
          console.error(`Model ${modelId} not found in models data`);
          message.showMessage("模型加载失败：找不到指定模型", 4000, 3);
          return;
        }
        ...
        message.showMessage("模型加载失败", 4000, 3);
      }
    }
    ...
    async loadRandModel() {
      try {
        ...
        const modelId = Number(localStorage.getItem("modelId")) || 1;
        const modelTexturesId = Number(localStorage.getItem("modelTexturesId")) || 1;
        ...
        // 获取纹理ID数组（处理对象格式）
        const textureIds = Object.keys(model.textures).map(Number);
        
        if (textureIds.length <= 1) {
          message.showMessage(this.#modelsData.message?.noOtherTextures || "我还没有其他衣服呢！", 4000, 3);
        } else {
          // 随机选择不同的纹理
          let nextTextureId;
          do {
            nextTextureId = textureIds[Math.floor(Math.random() * textureIds.length)];
          } while (nextTextureId === modelTexturesId && textureIds.length > 1);
          
          this.loadModel(modelId, nextTextureId, this.#modelsData.message?.newTexture || "我的新衣服好看嘛？");
        }
      } catch (error) {
        console.error('Error in loadRandModel:', error);
        message.showMessage("换装失败", 4000, 3);
      }
    }
    ...
    async loadOtherModel() {
      try {
        ...
        const modelIds = Object.keys(this.#modelsData.models).map(Number).sort((a, b) => a - b);
        const currentIndex = modelIds.indexOf(modelId);
        
        // 切换到下一个模型
        let nextIndex = (currentIndex + 1) % modelIds.length;
        let nextModelId = modelIds[nextIndex];
        
        const nextModel = this.#modelsData.models[nextModelId];
        if (nextModel) {
          this.loadModel(nextModelId, 1, nextModel.message);
        } else {
          console.error(`Next model ${nextModelId} not found`);
        }
      } catch (error) {
        console.error('Error in loadOtherModel:', error);
        message.showMessage("切换模型失败", 4000, 3);
      }
    }
  }
  ...
  tools.info = function (config = {}) {
    let siteUrl = "https://github.com/Aptx4869AC/plugin-live2d";
    return {
